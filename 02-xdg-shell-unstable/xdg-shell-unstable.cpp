...
static zxdg_shell_v6 *xdg_shell = nullptr;
static wl_shm *shm = nullptr;
static wl_compositor *compositor = nullptr;

static void handle_configure(void *data, zxdg_surface_v6 *surface, uint32_t serial) {
    zxdg_surface_v6_ack_configure(surface, serial);
}

static const zxdg_surface_v6_listener surface_listener = {
...
static void handle_global(void *data, wl_registry *registry,
    ...
    if (strcmp(interface, zxdg_shell_v6_interface.name) == 0) {
        xdg_shell = (zxdg_shell_v6*)wl_registry_bind(registry, name, &zxdg_shell_v6_interface, 1);
...
int main(int argc, char** argv) {
    ...
    zxdg_surface_v6 *shell_surface = zxdg_shell_v6_get_xdg_surface(xdg_shell, surface);
    zxdg_toplevel_v6 *toplevel = zxdg_surface_v6_get_toplevel(shell_surface);
    (void)toplevel;

    zxdg_surface_v6_add_listener(shell_surface, &surface_listener, nullptr);
